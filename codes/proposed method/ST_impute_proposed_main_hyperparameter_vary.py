...
*** This code also iterates the value of K and the threshold value to find the best parameters for higher clustering quality.
...
if __name__ == '__main__':
    ...
    # Parameters
    filename = 'GSE166692_scispace_processed'
    input_file = f"{filename}.h5ad"
    adata = sc.read_h5ad(input_file)
    adata_hvg = adata.copy()

    n_top = 5000  # Number of top genes to consider
    sc.pp.highly_variable_genes(adata_hvg, n_top_genes=n_top, flavor='seurat_v3')
    adata_hvg = adata_hvg[:, adata_hvg.var['highly_variable']]

    # Ranges to iterate
    k_range = [3, 5, 7, 9]
    threshold_range = np.arange(0.00, 0.41, 0.05)  # 0.05 to 0.35 inclusive
    results = []

    for k in k_range:
        for threshold in threshold_range:
            print(f"Evaluating for k={k}, threshold={threshold:.2f}...")
            result = evaluate_imputation_and_clustering(adata_hvg.copy(), k, threshold)
            results.append(result)

    # Save to CSV
    results_df = pd.DataFrame(results)
    results_df.to_csv(f"{filename}_all_genes_imputation_evaluation.csv", index=False)
